-- SANAD v0.1 clean-environment baseline.
--
-- Restored from commit 5378612 (the original executable
-- supabase/schema.sql). Later migrations 0002-0016 deliberately remain
-- unchanged and evolve this baseline in order. Existing hosted projects that
-- were bootstrapped from the historical schema must repair migration history
-- for version 0001 instead of executing this file blindly.
...
-- Requests: requester can create. Open requests are visible only to nearby,
-- explicitly available volunteers. Later migrations add presence staleness.
...
-- No direct client UPDATE policy is granted. State changes use RPCs.
